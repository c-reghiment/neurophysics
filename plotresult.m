% works for exercise 7

function plotresult(r,figno,strictaxis)
%function plotresult(r,figno,strictaxis)
%this function will plot the results of the simulations calc1 ... calc7
%parameters
%r     - structure with the results of the simulation
%figno - the figure number into which to plot
...
case 7 %{ML phase portrait}
    hf=figure; clf; setpaper(hf); 
    wscale=100;
    hold off
    quiver(r.V_grid,r.w_grid*wscale,r.V_grad,r.w_grad*wscale,'ko', ...
    'AutoScale','on','AutoScaleFactor',5,'Markersize',2); 
    hold on
    plot(r.V_lin,r.V_null*wscale,'b-', ...
         r.V_lin,r.w_null*wscale,'g-', ...    
         r.V,    r.w*wscale,     'ro-','Linewidth',3,'Markersize',5);
    ind=[ find(r.t>r.t_I1_start,1); ...
          find(r.t>r.t_I1_end,1);  ...
          find(r.t>r.t_I2_start,1); ...
          find(r.t>r.t_I2_end,1) ]; ...
    if r.I1>0 
        plot(r.V(ind(1:2)),r.w(ind(1:2))*wscale,'c*','Markersize',15); end
    if r.I2>0 
        plot(r.V(ind(3:4)),r.w(ind(3:4))*wscale,'m*','Markersize',15); end
    hold off;
